...
exercise* create_available_exercises(exercise* all_exercises, questionnaire user_questionnaire) {
    ...
    exercise* sorted_exercise_list = (exercise *)malloc (sizeof(exercise));
    int count=0;

    for (int n=0; n<10; n++) { // loop to cycle through exercise list
        int fail_switch = 0;
        for (int i=0; i<4; i++) { // Loop to cycle through exercise equipment also "length_of_equipment_enumn" istedet for 4
